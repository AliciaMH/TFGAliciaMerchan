...
const float LMin = -12.21855; // Corresponde a la distancia mínima, es decir, cuando el ángulo es -10 grados. En este caso de prueba se pone cuando el ángulo es -1 grado porque hay menos distancia de recorrido
...
unsigned long n_mediospasos = 0;      // Numero de medios pasos cuando el experimento empieza en -10 grados (-123.429mm)
...
float ang_i, ang_f = 0;               // Ángulos iniciales, será lo que elija el usuario en este experimento
...
void experimento() {
  ...
  if (inicio == 1) {
    ...
    if  (di_X ==LMin or di_X <= (0.0025*n_mediospasos + LMin)) { //Establezco los valores iniciales con distancia mínima negativa
...
void Micropasos() {
    if (estado == 3 && (avance_mediospasos + LMin < df_X) && fin == 0){//creo que habría que poner aquí tambien lo de arriba y en la función medios pasos también
      if (micropasos < 16){
...
void MedioPaso() {
  ...
  micropasos = 0;
    ...
    if ((0.0025*n_mediospasos + LMin > di_X)&& (avance_mediospasos + LMin < df_X)){  //No cuento los medios pasos hasta que no estoy en la distancia inicial.
    n_mediospasos2++;
    }
}
